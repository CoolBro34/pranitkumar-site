...
// nav-right-group-desktop-fix
document.addEventListener('DOMContentLoaded', () => {
  const rightGroup = document.querySelector('.nav-right-group');

  if (!rightGroup) return;

  const topbar = document.querySelector('.topbar');
  const mobileOverlayHeader =
    document.querySelector('.mobile-nav-header') ||
    document.querySelector('.mobile-overlay-header') ||
    document.querySelector('.mobile-menu-header');
  ...
  function relocateControls() {
    ...
    if (mobile) {
      if (mobileClose?.parentElement) {
        mobileClose.parentElement.appendChild(rightGroup);
      } else if (mobileOverlayHeader) {
        mobileOverlayHeader.appendChild(rightGroup);
      }

      rightGroup.classList.add('inside-mobile-menu');
    } else {
      topbar?.appendChild(rightGroup);
      rightGroup.classList.remove('inside-mobile-menu');
    }
  }
